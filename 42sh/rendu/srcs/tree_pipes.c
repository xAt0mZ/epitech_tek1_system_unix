...
** Last update Sun May 26 16:25:19 2013 marie lefebvre
*/
...
static int	tree_pipe_dup(t_tree *tree, int right_fd[], int infd, int pfd[])
{
...
int	tree_pipes(t_tree *tree, int infd, t_glob *glob, int files[])
{
  ...
  if (my_strcmp(tree->left->token, "<<") == 0)
    double_right_redir(tree->left, right_fd);
