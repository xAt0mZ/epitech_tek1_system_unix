...
** Last update Sun May 26 22:48:38 2013 damien sauvalle
*/

#include	<stdlib.h>
#include	"h42.h"

static char	**glob_tmp_allocation(char *str, char bool,
				      char word, char *param)
{
  ...
  if ((res = malloc((nb + 5) * sizeof(char *))) == NULL)
    return (NULL);
  i = 0;
  while (i < nb + 5)
    res[i++] = NULL;
...
static char	**cut_str_glob(char *str, char bool, char word, char *param)
{
  ...
  if (cpt[1] == 0 && (res[0] = my_strdup(str)) == NULL)
    return (NULL);
  return (res);
}
