...
** Last update Fri Mar  1 11:34:08 2013 louis-philippe baron
*/
...
int		built_setenv(char **command, t_list *list)
{
  int		i;
  int		retvalue;
  ...
  if (i > 3)
    ...
    built_env(command, list);
  else
    {
      i = 0;
      while (list->env[i] && my_strcmp(list->env[i], command[1]) == ERROR)
	i++;
      retvalue = modify_envarg(command, list->env, i);
    }
  return (retvalue);
}
